...
IUSE="authcram gencertdaily highvolume qmail-spp ssl vanilla outgoingips"
REQUIRED_USE='vanilla? ( !ssl !qmail-spp !highvolume !outgoingips )'
RESTRICT="test"
...
src_prepare() {
	...
	if ! use vanilla; then
		...
		use outgoingips && epatch "${FILESDIR}"/qmail-1.03_outgoingips.patch
	fi
