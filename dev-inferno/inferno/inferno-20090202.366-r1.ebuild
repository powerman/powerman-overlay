...
src_unpack() {
	...
	epatch "${FILESDIR}/${PV}/issue-122-csendalt.patch"
	epatch "${FILESDIR}/${PV}/issue-147-wait.patch"
	epatch "${FILESDIR}/${PV}/issue-162-flush.patch"
}
...
pkg_preinst() {
	enewgroup inferno || die
	enewuser inferno -1 -1 /usr/inferno/usr/inferno inferno || die
}

pkg_postinst() {
	chown -R inferno:inferno "${ROOT}"/usr/inferno
